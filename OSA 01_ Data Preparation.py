...
# MAGIC %md
# MAGIC The purpose of this notebook is to access and prepare the data needed for calculating on-shelf availability (OSA).  This notebook was developed on a Databricks 8.2 cluster by [Tredence](https://www.tredence.com/) in partnership with Databricks.
...
# MAGIC CREATE DATABASE IF NOT EXISTS osa ;
...
(
  spark
   ...
   .write
      ...
      .save('/mnt/osa/inventory_raw')
   )

# review data
display(spark.table('DELTA.`/mnt/osa/inventory_raw`'))
...
(
  spark
   .read
   .csv(
     '/mnt/osa/Vendor_Lead_Time_Info.csv',
     header = True,
...
start_date, end_date = (
  spark
  .table('DELTA.`/mnt/osa/inventory_raw`')
  .groupBy()
    .agg(
...
inventory_with_gaps = (
  dates
    ...
    .join(
      spark.table('DELTA.`/mnt/osa/inventory_raw`').drop('product_category'), 
